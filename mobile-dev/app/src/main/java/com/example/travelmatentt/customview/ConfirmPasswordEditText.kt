...
class ConfirmPasswordEditText: AppCompatEditText, View.OnTouchListener{
    ...
    private fun init() {
        ...
        passwordButtonImage = ContextCompat.getDrawable(context, if (!isPasswordVisible) R.drawable.remove_red_eye_24dp else R.drawable.hide_eye_24dp) as Drawable
        passwordIcon = ContextCompat.getDrawable(context, R.drawable.confirm_password_24dp) as Drawable
            ...
            if(isPasswordButtonClicked){
                when(event.action) {
                    MotionEvent.ACTION_DOWN -> {
                        passwordButtonImage = ContextCompat.getDrawable(context, if(isPasswordVisible) R.drawable.hide_eye_24dp else R.drawable.remove_red_eye_24dp) as Drawable
                        setEditCompoundDrawables(endOfTheText = passwordButtonImage, startOfTheText = passwordIcon)
